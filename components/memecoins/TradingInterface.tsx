...
import {calculateBuyCost, calculateSellProceeds} from '@/lib/trading.types';
...
export function TradingInterface({memecoin, userBalance}: TradingInterfaceProps) {
    ...
    const handleBuyAmountChange = (value: number) => {
        ...
        if (value > 0 && memecoin.supply >= 0) {
            const cost = calculateBuyCost(memecoin.supply, value);
            setBuyCost(cost);
        } else {
            setBuyCost(0);
        }
    ...
    const handleSellAmountChange = (value: number) => {
        ...
        if (value > 0 && memecoin.supply >= value) {
            const proceeds = calculateSellProceeds(memecoin.supply, value);
    ...
    // Initialize calculations on mount and when supply changes
    useEffect(() => {
        if (!isEffectRunning.current) {
            ...
            if (buyAmount > 0 && memecoin.supply >= 0) {
                const cost = calculateBuyCost(memecoin.supply, buyAmount);
                setBuyCost(cost);
            }

            // Calculate sell proceeds
            if (sellAmount > 0 && memecoin.supply >= sellAmount) {
                const proceeds = calculateSellProceeds(memecoin.supply, sellAmount);
                setSellProceeds(proceeds);
            }

            isEffectRunning.current = false;
        }
    }, [memecoin.supply, buyAmount, sellAmount]);
                    ...
                    <Tabs defaultValue="buy" className="w-full">
                        ...
                        <TabsContent value="buy" className="space-y-4">
                            ...
                            >
                                <motion.p 
                                    ...
                                    key={buyCost} // Re-animate when value changes
                                    initial={{ scale: 0.95 }}
                                    animate={{ scale: 1 }}
                        ...
                        <TabsContent value="sell" className="space-y-4">
                            ...
                            >
                                <motion.p 
                                    className="text-sm font-medium"
                                    key={sellProceeds} // Re-animate when value changes
                                    initial={{ scale: 0.95 }}
                                    animate={{ scale: 1 }}
                                    transition={{ duration: 0.2 }}
                                >
                                ...
                                <p className="text-xs text-muted-foreground">Available
                                    supply: {memecoin.supply.toLocaleString()} tokens</p>
                            </motion.div>
